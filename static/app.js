...
const OVERHEAD_FACTORY_BASE = `Machine-vision dataset photograph captured by an unseen high-mounted camera outside the frame. Request an 88 to 90 degree camera elevation to compensate for the generator's tendency to make the apparent angle too low, producing an apparent output angle of about 80 to 90 degrees. Keep the optical axis almost perpendicular to the work surface. Keep a realistic camera-to-workpiece working distance of 30 to 45 cm, preferably 35 to 40 cm, using a normal smartphone or inspection-camera lens rather than an ultra-wide lens. The horizontal work surface and local metal fixture fill the entire background edge-to-edge. There is no distant background and nothing hangs above the workpiece. The workpiece top face fills about 55 to 75 percent of the image, remains the dominant visible surface and appears close to its true plan-view shape with nearly parallel opposite edges. Side walls must occupy less than 10 percent of the visible workpiece height.`;
