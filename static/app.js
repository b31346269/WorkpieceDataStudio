...
const OVERHEAD_FACTORY_BASE = `Machine-vision dataset photograph captured by an unseen high-mounted camera outside the frame. Use an 80 to 90 degree elevation above the conveyor, preferably near 85 degrees, keeping the optical axis close to perpendicular to the work surface. Keep a realistic camera-to-workpiece working distance of 30 to 45 cm, preferably 35 to 40 cm, using a normal smartphone or inspection-camera lens rather than an ultra-wide lens. The horizontal conveyor belt and metal inspection fixture fill the entire background edge-to-edge. There is no distant background and nothing hangs above the workpiece. The workpiece top face fills about 55 to 75 percent of the image, remains the dominant visible surface and appears close to its true plan-view shape with nearly parallel opposite edges. A shallow side rim may be visible for realistic depth, but it must occupy only a small fraction of the workpiece and must never dominate the image.`;
...
const OVERHEAD_COMPOSITION = `Composition is mandatory: high-angle overhead inspection frame between 80 and 90 degrees, preferably near 85 degrees, top surface facing the viewer, only limited shallow side visibility, no low oblique angle, no three-quarter product view, no horizon, no strong vanishing point, no ceiling, no walls, no shelves, no distant machinery, and no camera, spindle, probe, lamp or inspection head anywhere inside the frame.`;
...
const WORKPIECE_RECIPES = {
  ...
  scale_light_fixture: `Create a mechanically plausible cast-aluminum gearbox housing with strongly varied physical proportions and framing while strictly preserving the 80 to 90 degree overhead viewpoint, preferably near 85 degrees. Alternate among compact square, elongated rectangular and irregular housings; vary the workpiece occupancy from about 45 to 80 percent of the image and rotate it naturally on the fixture without increasing camera obliqueness. Use believable conveyor pallets, clamping rails, locating pins or worn metal inspection plates. Vary dim ambient light, localized point light from outside the frame, mixed cool factory light, partial underexposure, hard contact shadows and metallic highlights while keeping holes and fasteners clearly reviewable.`,
};

const GENERATION_NEGATIVE = "camera closer than 30 cm, camera farther than 45 cm, ultra-wide lens, fisheye distortion, camera elevation below 80 degrees, low angle, eye level, strongly oblique camera, three-quarter product photo, large visible front face, large visible side face, side-dominant composition, strong trapezoid perspective, horizon, strong vanishing point, ceiling, walls, shelves, distant factory interior, visible camera, hanging camera head, spindle, probe, lamp, inspection head";
