...
const OFFSET_MAINTENANCE_PROMPT = `Near-overhead smartphone maintenance-inspection photo. Create a different used, disassembled shallow cast-aluminum gearbox end cover or bearing housing, not a pristine sealed product casing and not a tall cubic box. Favor a low-profile circular, rounded, polygonal or irregular plate-like body with uneven mounting ears and restrained asymmetric ribs. Place one substantial machined bearing flange moderately off-center toward one side or corner. The dull aluminum must show natural machining marks, fine scratches, handling grime, light oxidation, grease residue, small casting imperfections, uneven wear and imperfect workshop illumination so it belongs naturally on the surrounding used bench. Preserve high variation in the silhouette, flange position, rib layout and mounting ears, but avoid an overly smooth, perfectly uniform, toy-like or freshly rendered surface. Show only 2 to 4 small holes and 1 to 2 fully seated recessed-drive screws. Keep the complete workpiece visible from a near-overhead angle with only a thin side-wall rim.`;
...
const WORKPIECE_RECIPES = {
  ...
  offset_flange: `Create a clearly different but mechanically plausible low-profile cast-aluminum gearbox housing whose defining feature is one substantial machined bearing seat, flange or shaft interface positioned decisively off-center toward one randomly selected side or corner. Keep the flange center at least one full flange radius away from the housing's geometric center, and vary its diameter, surrounding boss shape and local bolt pattern between images. The geometric center must remain solid cast metal, an asymmetric rib junction or a shallow rectangular recessed panel; never place the dominant circular opening at the geometric center. Build a clearly asymmetric external silhouette with unequal mounting ears, non-mirrored reinforcement ribs that connect naturally to the offset flange, offset cast cavities and cooling fins distributed more heavily on one side. Keep the housing shallow, with only a thin side-wall rim visible from overhead. Preserve broad uncluttered top-face areas and mechanically plausible load paths; do not create bilateral symmetry, a centered-flange layout or one repeated housing design.`,
